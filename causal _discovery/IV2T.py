...
def load_data():
    ...
    xingfa_dic_path = addr + "code/explanation_project/explanation_model/models_v2/data/xingfa_law_dic.json"
    with open(xingfa_dic_path, 'r') as f:
        xingfa_dic = json.load(f)
        laws_selected = ["中华人民共和国刑法" + item for item in xingfa_dic.keys()]  # todo

    xingfa_list_path = addr + "code/explanation_project/explanation_model/models_v2/data/xingfa_law_list.json"
    with open(xingfa_list_path, 'r') as f:
        xingfa_list = json.load(f)
        laws_selected_section_num = []
        for item1 in xingfa_list:
            n = 0
            for item2 in item1:
                n += len(item2)
...
def dump_data(combine_sentence, laws_selected, y, factor, factor_embedding, idx, word_idx, word_key, sentence_fact_laws, law_type, section_idx):
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_name'.format(law_type, section_idx) + '.json', 'w') as f:
        json.dump(name_contain_dict, f, ensure_ascii=False)


    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}'.format(law_type, section_idx) + '.tsv', 'w') as f:
        for i in range(len(combine_sentence)):
            f.write('x' + str(i) + '\t')
        for i in range(len(laws_selected)):
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_embedding'.format(law_type, section_idx) + '.tsv', 'w') as f:
        for i in range(len(combine_sentence)):
            f.write('x' + str(i) + '\t')
        for i in range(len(laws_selected)):
            if i < len(laws_selected) - 1:
    ...
    """
        将law对应的sentence保存下来
    """
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_fact4laws'.format(law_type, section_idx) + '.json', 'w') as f:
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_data_{}'.format(law_type, section_idx) + '.pkl', 'wb') as f:
        pkl.dump(data_new, f)
...
def build_causal_graph(law_type, section_idx):
    ...
    """
    factors = pd.read_csv(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_embedding'.format(law_type, section_idx) + '.tsv', delimiter='\t')
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_fact4laws'.format(law_type, section_idx) + '.json', 'r') as f:
        fact4law = json.load(f)

    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_name'.format(law_type, section_idx) + '.json', 'r') as f:
        name = json.load(f)
    count_x, count_y =0, 0
    ...
    try:
        ...
        for i in range(count_y):
            item = name['y'+str(i)]
            final_output[item] = [xingfa_dic[item[9:]], fact4law[item]]
            nodes_list = []
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_final_data'.format(law_type, section_idx) + '.json', 'w') as f:
...
def convert_data2embedding(law_type, section_idx):
    import copy
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_final_data'.format(law_type, section_idx) + '.json', 'r') as f:
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_embedding4laws'.format(law_type, section_idx) + '.json', 'w') as f:
...
def cat2onefile(law_type, laws_selected_section_num):
    ...
    for section_idx, num in enumerate(laws_selected_section_num):
        with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/{}_{}_embedding4laws'.format(
    ...
    with open(addr + '/code/explanation_project/explanation_model/models_v2/data/GCI/Casual_Law.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    if new_start:
        for idk, num in enumerate(laws_selected_section_num):
            ...
            dump_data(combine_sentence, laws_selected, y, factor, factor_embedding, idx, word_idx, word_key, sentence_fact_laws, "xingfa", idk)
            begin = end
    else:
        # for idk, num in enumerate(laws_selected_section_num):
        #     print("~~~~~", idk, "~~~~~~~~")
            #build_causal_graph("xingfa", idk)
            #convert_data2embedding("xingfa", idk)
        cat2onefile("xingfa", laws_selected_section_num)
