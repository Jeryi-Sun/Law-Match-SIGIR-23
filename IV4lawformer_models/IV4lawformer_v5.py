...
parser.add_argument('--batch_size_train', type=int, default=4, help='batch size')
parser.add_argument('--batch_size_test', type=int, default=4, help='batch size')
...
parser.add_argument('--early_stopping_patience', type=int, default=10, help='whether train')
...
parser.add_argument('--log_name', type=str, default="IV4sentence_bert_V5", help='whether train')
...
parser.add_argument('--together_type', type=str, default='attention', help='[attention, mlp]')
...
class Collate:
    ...
    def __call__(self, batch):
        text, text_z, labels = [], [], []
        for item in batch:
            text.append([item[0][:self.max_seq_len//2], item[1][:self.max_seq_len//2]])
            text_z.append([item[2][:self.max_seq_len//2], item[3][:self.max_seq_len//2]])
            labels.append(item[4])
        dic_data = self.tokenizer.batch_encode_plus(text, padding=True, truncation=True,
                                                      max_length=self.max_seq_len, return_tensors='pt')
        dic_data_z = self.tokenizer.batch_encode_plus(text_z, padding=True, truncation=True,
                                                    max_length=self.max_seq_len, return_tensors='pt')
        return dic_data, dic_data_z, torch.tensor(labels)
...
class PredictorModel(nn.Module):
    ...
    def separate(self, text):
        hidden_states_text = self.model(**text)[0]
        text_A_mask = ((text['token_type_ids'] * -1 + 1) * text['attention_mask'])
        text_A_mask[:, 0] = 0  # 去掉 [CLS]
        text_B_mask = text['token_type_ids']
        text_embedding_A = torch.sum(hidden_states_text * text_A_mask.unsqueeze(-1), dim=-2) / torch.sum(text_A_mask, dim=-1, keepdim=True)
        text_embedding_B = torch.sum(hidden_states_text * text_B_mask.unsqueeze(-1), dim=-2) / torch.sum(text_B_mask, dim=-1, keepdim=True)
        return text_embedding_A, text_embedding_B
    def forward(self, text, IV_model, text_batch_z, IV_stage_1=False):
        output_text_a, output_text_b = self.separate(text)
        output_text_z_a, output_text_z_b = self.separate(text_batch_z)
        if IV_stage_1:
            """
               是否对PLM进行更新
            """
            if args.PLM_update_s1:
                s1_loss_A, s1_loss_B = IV_model(output_text_z_a, output_text_z_b,
                                                output_text_a, output_text_b, stage_1=IV_stage_1)
            else:
                s1_loss_A, s1_loss_B = IV_model(output_text_z_a.detach(), output_text_z_b.detach(),
                                                output_text_a.detach(), output_text_b.detach(), stage_1=IV_stage_1)
            return s1_loss_A, s1_loss_B

        user_emb_A, user_emb_B, s1_loss_A, s1_loss_B = IV_model(output_text_z_a, output_text_z_b, output_text_a,
                                                                output_text_b)
        #data_p = torch.cat([user_emb_A, user_emb_B, torch.abs(user_emb_A - user_emb_B)], dim=-1)
        data_p = torch.cat([user_emb_A, user_emb_B], dim=-1)
        output = self.FFN(data_p)

        return output, s1_loss_A, s1_loss_B
...
def train_valid(model, IV_model, train_dataloader, valid_dataloader, test_dataloader):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_data_stage_1 in pbar:
            text_batch, text_batch_z, label_batch = batch_data_stage_1
            text_batch = text_batch.to(device)
            text_batch_z = text_batch_z.to(device)
            s1_loss_A, s1_loss_B = model(text_batch, IV_model, text_batch_z, IV_stage_1=True)
        ...
        for batch_data in pbar:
            text_batch, text_batch_z, label_batch = batch_data
            text_batch = text_batch.to(device)
            text_batch_z = text_batch_z.to(device)
            label_batch = label_batch.to(device)
            scores, s1_loss_A, s1_loss_B = model(text_batch, IV_model, text_batch_z)
            ...
            pbar.set_description(
                "train loss {:.4f} IV_loss {:.4f} ".format(epoch_loss / current_step, IV_loss / current_step))
            if current_step % 100 == 0:
                logging.info(
                    "train step {} loss {:.4f}  IV_loss {:.4f} ".format(current_step, epoch_loss / current_step,
                                                                        IV_loss / current_step))
        ...
        if is_save:
            save_checkpoint(model, optimizer, epoch, "IV4lawformer_model")
            save_checkpoint(IV_model, optimizer, epoch, "IV4lawformer_IV_net")
        else:
            pass
...
def evaluation(valid_dataloader, model, IV_model, epoch, type='valid'):
    with torch.no_grad():
        ...
        for batch_data in pbar:
            ...
            text_batch_z = text_batch_z.to(device)
            label_batch = label_batch.to(device)
            scores, _, _ = model(text_batch, IV_model, text_batch_z)
        ...
        time_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(colored('{} {} epoch {} acc {}/{}={:.4f}'.format(time_str, type, epoch, correct, total, correct / total),
                      'red'))
        logging.info('{} epoch {} acc {}/{}={:.4f}'.format(type, epoch, correct, total, correct / total))
