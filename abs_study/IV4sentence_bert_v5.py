...
def load_data(filename):
    ...
    with open(filename) as f:
        for l in f:
            item = json.loads(l)
            if args.rationale:
                item["case_a"] = item["case_a_rationale"]
                item["case_b"] = item["case_a_rationale"]
...
class Collate:
    ...
    def __call__(self, batch):
        text_a, text_b, text_z_a, text_z_b, labels = [], [], [], [], []
        for item in batch:
        ...
        dic_data_z_b = self.tokenizer.batch_encode_plus(text_z_b, padding=True, truncation=True,
                                                    max_length=self.max_seq_len, return_tensors='pt')
        return dic_data_a, dic_data_b, dic_data_z_a, dic_data_z_b, torch.tensor(labels)
...
class PredictorModel(nn.Module):
    ...
    def forward(self, text_a, text_b, IV_model, text_batch_z_a, text_batch_z_b, IV_stage_1=False):
        ...
        output_text_z_a = self.model(**text_batch_z_a).pooler_output
        output_text_z_b = self.model(**text_batch_z_b).pooler_output
        if IV_stage_1:
            """
               是否对PLM进行更新
            """
            if args.PLM_update_s1:
                ...
                s1_loss_A, s1_loss_B = IV_model(output_text_z_a.detach(), output_text_z_b.detach(), output_text_a.detach(), output_text_b.detach(), stage_1=IV_stage_1)
            return s1_loss_A, s1_loss_B

        user_emb_A, user_emb_B, s1_loss_A, s1_loss_B = IV_model(output_text_z_a, output_text_z_b, output_text_a, output_text_b)
        if args.save_embedding:
            return user_emb_A, user_emb_B
...
def train_valid(model, IV_model, train_dataloader, valid_dataloader, test_dataloader):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_data_stage_1 in pbar:
            ...
            text_batch_z_a = text_batch_z_a.to(device)
            text_batch_z_b = text_batch_z_b.to(device)
        ...
        if is_save:
            save_checkpoint(model, optimizer, epoch, "IV4sentence_bert_model")
            save_checkpoint(IV_model, optimizer, epoch, "IV4sentence_bert_IV_net")
        else:
            pass
    ...
    with torch.no_grad():
        ...
        if args.save_embedding:
            ...
            for batch_data in pbar:
                ...
                text_batch_z_b = text_batch_z_b.to(device)
                label_batch = label_batch.to(device)
                user_emb_A, user_emb_B = model(text_batch_a, text_batch_b, IV_model, text_batch_z_a, text_batch_z_b)
                data_dic.append({"user_emb_A": user_emb_A.cpu().tolist(), "user_emb_B": user_emb_B.cpu().tolist(),
                                 "label": label_batch.cpu().tolist()})
                if args.rationale:
                    with open("IV4sentence_bert_embedding_rationale.json", "w") as f:
                        json.dump(data_dic, f, ensure_ascii=False)
                else:
                    with open("IV4sentence_bert_embedding.json", "w") as f:
                        json.dump(data_dic, f, ensure_ascii=False)
            return 0
        for batch_data in pbar:
            text_batch_a, text_batch_b, text_batch_z_a, text_batch_z_b, label_batch = batch_data
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.update_treatment_type == 'agg':
            P_model_path = "/home/zhongxiang_sun/code/explanation_project/explanation_model/models_for_paper/transfer_weight/IV4sentence_bert_model_agg.pkl"
            IV_model_path = "/home/zhongxiang_sun/code/explanation_project/explanation_model/models_for_paper/transfer_weight/IV4sentence_bert_IV_net_agg.pkl"
        elif args.update_treatment_type == 'Sth':
            P_model_path = "/home/zhongxiang_sun/code/explanation_project/explanation_model/models_for_paper/transfer_weight/IV4sentence_bert_model_Sth.pkl"
            IV_model_path = "/home/zhongxiang_sun/code/explanation_project/explanation_model/models_for_paper/transfer_weight/IV4sentence_bert_IV_net_Sth.pkl"
        load_checkpoint(P_model, P_model_path)
        load_checkpoint(IV_model, IV_model_path)
        with torch.no_grad():
            P_model.eval()
            IV_model.eval()
            evaluation(all_data_loader, P_model, IV_model, 0, "all")
